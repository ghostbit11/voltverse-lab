...
function HINTS(): array {
    return [
      ...
      'graphql-introspect' => ["GraphQL can describe itself.","Ask it for <code>__schema</code> to dump every type and field."],
      'graphql-bola' => ["Objects are fetched by id.","Resolve <code>user(id: 2)</code> — an object that isn't yours."],
      'graphql-sqli' => ["A resolver argument reaches SQL unsanitised.","Inject a UNION through <code>products(filter: \"…\")</code>."],
      'oauth-redirect' => ["The authorize endpoint trusts <code>redirect_uri</code>.","Change it to an external URL you control and watch the code follow."],
      'oauth-state'  => ["OAuth uses <code>state</code> to stop CSRF.","What happens if the callback receives no state at all?"],
      'deserial-poi' => ["The prefs blob is a serialized PHP object.","Decode it, bump a field to something privileged, re-encode — think <code>role=admin</code>."],
